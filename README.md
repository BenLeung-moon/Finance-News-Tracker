...
Additional themes are added as new Profile modules under
`src/finance_news_tracker/profiles/` (see “Create a new Profile” below).
额外主题以新的 Profile 模块形式添加（见下文「新建完整 Profile」）。
...
### Override lookback / 临时覆盖回看窗口

```powershell
$env:TRACKER_PROFILE = "usdjpy"
$env:RECENCY_HOURS = "48"
python -m finance_news_tracker collect
```

Leave `RECENCY_HOURS` unset to use the active profile default (72 hours for
`usdjpy`).
`RECENCY_HOURS` 未设置时使用当前 Profile 默认值（`usdjpy` 为 72 小时）。
...
Sources belong to a profile's `SOURCES` list. The built-in example is:

- `src/finance_news_tracker/profiles/usdjpy.py`

数据源定义在各 Profile 的 `SOURCES` 列表中；可直接参考内置的 `usdjpy` 示例。
...
as a JSON API, POST-based archive, or authenticated internal feed.
...
3. Use the base summary style or mount a custom `SummaryProfile` for sections
   and labels (`profiles/summary_base.py` is the starting point).
