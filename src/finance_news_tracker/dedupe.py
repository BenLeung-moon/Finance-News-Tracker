...
def cross_language_articles_similar(
    left: Article,
    right: Article,
    settings: Settings,
) -> bool:
    """Conservatively match profile-configured source pairs on facts.

    Uses ``TrackerProfile.cross_language_source_pairs`` (default empty = off).
    中文注解：日英文本无法由字符相似度可靠比较；只在 profile 声明的来源对上，
    且发布时间接近并有共享实体/容量等可验证事实时，才视为同一事件。
    """
    pair = frozenset({left.source, right.source})
    if pair not in settings.active_profile.cross_language_source_pairs:
        return False
...
def articles_similar(
    ...
    if text_similarity(left.title, right.title, settings) >= threshold:
        return True
    return cross_language_articles_similar(left, right, settings)
