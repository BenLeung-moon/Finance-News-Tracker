...
class SourceEntityBoostRule:
    ...
    Bonus applies only when ``article.source == source_id`` and text matches
    an alias. Does NOT alter LLM ``relevance_score`` or force prefilter pass.
...
class TrackerProfile:
    ...
    # Source-scoped entity ranking boosts; empty for most profiles.
    source_entity_boost_rules: list[SourceEntityBoostRule] = field(default_factory=list)
    # Exact source-id pairs eligible for fact-based cross-language dedupe.
    # Each entry is a frozenset of two source ids. Empty = feature disabled.
    # 中文注解：跨语言去重的来源对；默认空表示关闭该特判。
    cross_language_source_pairs: frozenset[frozenset[str]] = field(default_factory=frozenset)
