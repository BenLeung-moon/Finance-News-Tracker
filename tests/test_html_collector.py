...
def test_catl_selector_extracts_clean_title_and_date():
    source = SourceConfig(
        id="catl_news",
        name="CATL (News)",
        kind="html",
      ...
      <ul>
        ...
        <li class="mc_e1_li">
          <a href="/en/about/" class="mc_e1_lisbox">
            <p class="mc_e1_txt">About CATL navigation link</p>
          </a>
        </li>
        <li class="mc_e1_li">
          <a href="/en/news/6942.html" class="mc_e1_lisbox">
    ...
    """
    soup = BeautifulSoup(html, "lxml")
    items = _extract_from_page(soup, source, "https://www.catl.com/en/news/")
    assert len(items) == 1
    assert items[0].title == "Brunp Wins Two Honors at the European Inventor Award 2026"
    assert items[0].url == "https://www.catl.com/en/news/6942.html"
    assert items[0].source == "catl_news"
    assert items[0].published_at is not None
    assert items[0].published_at.year == 2026
...
def test_hithium_selector_extracts_clean_title_and_date():
    source = SourceConfig(
        id="hithium_latest_updates",
        name="HiTHIUM (Latest Updates)",
        kind="html",
        url="https://www.hithium.com/newsroom/LatestUpdates",
        languages=["en"],
    ...
    <html><body>
      <div class="new-list view">
        <a href="https://www.hithium.com/newsroom/latest/details/461.html">
          <div class="txt">
            <div class="time">July                                28,2026</div>
            <h1 class="h4-b t1">
              HiTHIUM Supports Statkraft's Renewable Expansion with First Project in Navarra
            </h1>
            <div class="vmore"><div class="mc">view details</div></div>
          </div>
        </a>
        <a href="https://www.hithium.com/newsroom/latest/details/446.html">
          <div class="txt">
            <div class="time">July                                02,2026</div>
            <h1 class="h2-s-b text-uppercase">
              HiTHIUM Navarra Open Day: Cobuilding Spain's Local Industrial Ecosystem
            </h1>
          </div>
        </a>
        <a href="https://www.hithium.com/newsroom/LatestUpdates/2.html">
          <div class="txt">
    ...
    soup = BeautifulSoup(html, "lxml")
    items = _extract_from_page(
        soup, source, "https://www.hithium.com/newsroom/LatestUpdates"
    )
    assert len(items) == 2
    assert items[0].title.startswith("HiTHIUM Supports Statkraft")
    assert "view details" not in items[0].title.lower()
    assert "July" not in items[0].title
    assert items[1].title.startswith("HiTHIUM Navarra Open Day")
